...
import numpy as np
from mocasin.util.random_distributions.discrete_random import (
    _discrete_gauss_plain,
    _discrete_uniform_plain,
    _discrete_random,
)


def template_test_plain_distribution(
    distribution_func, dims, eigens, num_executions=1000
):
    np.random.seed(0)
    randoms = []
...
def test_discrete_uniform(ns, eigenv):
    np.random.seed(0)
    mean, std, _ = template_test_plain_distribution(
        _discrete_uniform_plain, ns, eigenv, 100000
    )
    expected_std = [np.sqrt(((b - 0 + 1) ** 2 - 1) / 12) for b in eigenv.flat]
    assert np.allclose(mean, eigenv / 2.0, atol=0.5)
    assert np.allclose(std, expected_std, atol=0.5)


def test_discrete_gauss(ns, eigenv):
    np.random.seed(0)
    mean, std, median_vec = template_test_plain_distribution(
        _discrete_gauss_plain, ns, eigenv, 100000
    )
