...
from mocasin.tgff.tgffParser import regEx as expr
...
class Parser:
    ...
    def parse_file(self, file_path, proc_type_list):
        ...
        with open(file_path, "r") as file:
            ...
            while current_line:
                key, match = self._parse_line(
                    current_line, self.data_components
                )
                if key == "task_graph":
                    ...
                    self._parse_hw_component(
                        file, match, last_mismatch, proc_type_list
                    )
    ...
    def _parse_task_graph(self, file, match):
        ...
        while current_line:
            key, match = self._parse_line(
                current_line, self.task_graph_components
            )
            if key == "task":
                self.logger.debug(
                    "Added task: "
                ...
                if not match.group("name") in tasks:
                    tasks.update(
                        {match.group("name"): int(match.group("type"))}
                    )
                else:
                    tasks.update(
                        {
                            match.group("name")
                            + "_"
                            + str(task_index_extension): match.group("type")
                        }
                    )
        ...
        self.task_graph_dict.update(
            {
                identifier: TgffGraph(
                    identifier, tasks, channels, self.quantity_dict
                )
            }
        )
    ...
    def _parse_commun_quant(self, file, match):
        ...
        while current_line:
            key, match = self._parse_line(
                current_line, self.commun_quant_components
            )
            if key == "commun_value":
                self.logger.debug(
                    "Added commun_value "
                ...
                commun_values.update(
                    {
                        int(match.group("identifier")): float(
                            match.group("value")
                        )
                    }
                )
    ...
    def _parse_hw_component(self, file, match, last_missmatch, proc_type_list):
        ...
        while current_line:
            ...
            if key == "std_link_value":
                self._parse_std_link(
                    identifier, file, match, lower_last_missmatch
                )
                return
                ...
                self._parse_processor(
                    identifier, file, key, match, proc_type_list
                )
                return
            elif key == "scope_limiter":
                self.logger.error(
                    "Reached end of scope. Unable to recognize HW component!"
                )
            else:
                self._key_mismatch(key, file.tell())
    ...
    def _parse_processor(self, identifier, file, key, match, proc_type_list):
        ...
        self.processor_list.append(
            TgffProcessor(identifier, operations, processor_type=(name))
        )
