...
class TraceSegment(object):
    ...
    def sanity_check(self):
        ...
        if (
            self.read_from_channel is not None
            and self.write_to_channel is not None
        ):
            raise RuntimeError(
                "A trace segment may not contain both a read "
                "and a write access"
            )
        if self.n_tokens is None and (
            self.read_from_channel is not None
            or self.write_to_channel is not None
        ):
            raise RuntimeError(
                "Trace segments contains a channel access but "
