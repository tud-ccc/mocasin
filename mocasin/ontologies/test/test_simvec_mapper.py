...
def testMappingGeneration5(map_dict_solver):
    input_query = (
        "EXISTS RUNNING TOGETHER [src, fft_l, fft_r, filter_l, filter_r, ifft_l, ifft_r, sink ]"
        "AND EQUALS map_two"
    )
...
def testSetVector3(map_dict_solver):
    input_query = (
        "EXISTS RUNNING TOGETHER [src, fft_l, fft_r, filter_l, filter_r, ifft_l, ifft_r, sink ] AND EQUALS"
        "map_two"
    )
