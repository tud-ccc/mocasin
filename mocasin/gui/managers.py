...
class drawManager:
    ...
    def __init__(
        self, apiInstance, canvas, border, scaling, width, height, **kwargs
    ):
    ...
    def drawMappings(self):
        ...
        for coreName in usedCores:
            startPointX = self.__mApiInstance.getPlatform().getCoreDict()[
                coreName
            ][0]
            startPointY = self.__mApiInstance.getPlatform().getCoreDict()[
                coreName
            ][2]
            endPointX = self.__mApiInstance.getPlatform().getCoreDict()[
                coreName
            ][1]
            endPointY = self.__mApiInstance.getPlatform().getCoreDict()[
                coreName
            ][3]

            self.__drawnCrosses.append(
                self.__mCanvas.create_line(
                    startPointX,
                    startPointY,
                    endPointX,
                    endPointY,
        ...
        for key in coreUsage:
            if bool(coreUsage[key]):
                ...
                for innerKey in coreUsage[key]:
                    color = self.__mApiInstance.getMappings()[
                        innerKey
                    ].getColor()
                    for entry in coreUsage[key][innerKey]:
                        if (currentX + radius) > endPointX:
                            currentY += 2 * radius
                            currentX = startPointX + radius
                        circleHandle = self.__mCanvas.create_oval(
                            currentX - radius,
                            currentY - radius,
                            currentX + radius,
                            currentY + radius,
                        ...
                        self.__mApiInstance.getMappings()[
                            innerKey
                        ].addCircleHandle(circleHandle)

                        nameHandle = None
                        if self.__displayTaskNames:
                            if self.__fontSize != "default" and isinstance(
                                self.__fontSize, int
                            ):
                                nameHandle = self.__mCanvas.create_text(
                                    currentX,
                                    currentY,
                                    font=("Helvetica", self.__fontSize),
                                    text=entry,
                                )
                            else:
                                nameHandle = self.__mCanvas.create_text(
                                    currentX, currentY, text=entry
                                )
                            self.__mApiInstance.getMappings()[
                                innerKey
                            ].addNameHandle(nameHandle, entry)
                        else:
                            if self.__fontSize != "default" and isinstance(
                                self.__fontSize, int
                            ):
                        ...
                        if self.__enableDragAndDrop and nameHandle != None:
                            self.__mCanvas.tag_bind(
                                circleHandle,
                                "<ButtonPress-1>",
                                self.__mDragAndDropManager.onPressDot,
                            )
                            self.__mCanvas.tag_bind(
                                nameHandle,
                                "<ButtonPress-1>",
                                self.__mDragAndDropManager.onPressName,
                            )
                        currentX += 2 * radius
    ...
    def clearMappings(self):
        ...
        try:
            ...
            if not coreDict == None:
                for key in coreDict:
                    self.__mCanvas.itemconfig(
                        coreDict[key][4], fill=coreDict[key][5]
                    )
        except:
            pass
        ...
        for key in self.__mApiInstance.getMappings():
            ...
            for innerKey in mappingDescription:
                if len(mappingDescription[innerKey]) == 0:
                    ...
                    coreUsage[innerKey].update(
                        {mapping.getMappingId(): mappingDescription[innerKey]}
                    )
    ...
    def __resolvePlatformColor(self, colorValue):
        ...
        """
        if (
            isinstance(colorValue, int)
            and colorValue >= 0
            and colorValue <= len(self.__platformColors)
        ):
            return self.__platformColors[colorValue]
        else:
            return self.__resolvePlatformColor(
                colorValue % len(self.__platformColors)
            )
    ...
    def __drawInnerRessource(
        self, toDraw, drawWidth, drawHeight, relativeXValue, xIndent, colorValue
    ):
        ...
        self.__mCanvas.create_rectangle(
            startPointX, startPointY, endPointX, endPointY, fill=color
        )

        if self.__fontSize != "default" and isinstance(self.__fontSize, int):
            self.__mCanvas.create_text(
                textPointX,
                textPointY,
                font=("Helevetica", self.__fontSize),
                text=toDraw[0],
                width=length,
            )
        else:
            self.__mCanvas.create_text(
                textPointX, textPointY, text=toDraw[0], width=length
            )
        ...
        if drawPEs or drawPEsWithPrimitive:
            ...
            if drawPEs and (toDraw[0] == "network_on_chip"):
                self.__drawPEs(
                    ...
                    restSizeY,
                    relativeXValue,
                    xIndent + relativeXValue * (length + border),
                    yIndent,
                    True,
                )
            elif drawPEs:
                ...
                self.__drawPEsWithPrimitive(
                    ...
                    xIndent + relativeXValue * (length + border),
                    yIndent,
                    toDraw[0].split("_")[len(toDraw[0].split("_")) - 1],
                )
            else:
                raise (
                    RuntimeWarning(
                        "There is no known primitive Structure to draw"
                    )
                )
            ...
            i = 0
            for item in toDraw[1]:
                self.__drawInnerRessource(
                    item,
                    restSizeX,
                    restSizeY,
                    i,
                    relativeXValue * drawWidth,
                    nextColor,
                )
                i += 1
        return

    def __skipInnerRessource(
        self, toDraw, drawWidth, drawHeight, relativeXValue, xIndent, colorValue
    ):
        """
        :param list[(str, [str])] toDraw: List that contains tuples with first the name of the primitive an second a list of primitives and or processing
    ...
    def __drawPEsWithPrimitive(
        ...
        if (sizePEY - self.__textSpace) < sizeX and (
            sizePEY - self.__textSpace
        ) < self.__minimalPeSize:
        ...
        for row in matrix:
            ...
            textPointPrimitiveY = (
                endPointPrimitiveY
                - (endPointPrimitiveY - startPointPrimitiveY) / 2
            )
            ...
            for item in row:
                ...
                if coreName == "A15":
                    ...
                    color = "antique white"
                peHandle = self.__mCanvas.create_rectangle(
                    startPointX,
                    startPointPEY,
                    endPointX,
                    endPointPEY,
                ...
                if (
                    self.__fontSize != "default"
                    and isinstance(self.__fontSize, int)
                    and self.__displayPENames
                ):
                    self.__mCanvas.create_text(
                        textPointX,
                        textPointPEY,
                    ...
                    self.__mCanvas.create_text(
                        textPointX, textPointPEY, text=item[1][0], width=sizeX
                    )
                platform.updateCoreDict(
                    item[1][0],
                    [
                        startPointX,
                        endPointX,
    ...
    def __drawPEs(
        ...
        if (sizeY - self.__textSpace) < sizeX and (
            sizeY - self.__textSpace
        ) < self.__minimalPeSize:
        ...
        for row in matrix:
            ...
            for item in row:
                ...
                if (
                    ...
                    self.__mCanvas.create_text(
                        textPointX, textPointY, text=item, width=sizeX
                    )
                platform.updateCoreDict(
                    item,
                    [
    ...
    def drawPlatform(self):
        ...
        for slot in self.__layout.getLayout():
            if not slot[0]:
                ...
                self.drawProcessingElements(
                    slot[2]
                    + self.__currentPaddingLevel * self.__clusterPadding,
                    slot[3]
                    + self.__currentPaddingLevel * self.__clusterPadding,
                    slot[4]
                    - self.__currentPaddingLevel * self.__clusterPadding,
    ...
    def drawProcessingElements(self, startX, startY, endX, endY, clusterObject):
        ...
        if clusterObject[4]:
            ...
            self.draw_rectangle(
                startX, startY, endX, endY, fill=self.__resolvePlatformColor(1)
            )
        ...
        for i in range(0, dimension):
            ...
            for j in range(0, dimension):
                ...
                if self.__displayPENames:
                    self.__mCanvas.create_text(
                        leftCornerX + 0.5 * peSizeX,
                        rightCornerY - self.__textSpace,
                        text=matrix[i][j],
                    )
                ...
                self.__mApiInstance.getPlatform().updateCoreDict(
                    matrix[i][j],
                    [
                        leftCornerX,
                        rightCornerX,
                ...
                if clusterObject[2]:
                    ...
                    if i < dimension - 1:
                        ...
                        self.__mCanvas.create_line(
                            lineX, lineStartY, lineX, lineEndY
                        )
                    ...
                    if j < dimension - 1:
                        ...
                        self.__mCanvas.create_line(
                            lineStartX, lineY, lineEndX, lineY
                        )
    ...
    def draw_rectangle(self, x1, y1, x2, y2, fill="red", outline=False):
        ...
        if outline:
            self.__mCanvas.create_polygon(
                points, fill=fill, smooth=True, outline="black"
            )
        else:
...
class dragAndDropManager:
    ...
    def onPressDot(self, event):
        ...
        if not self.__draggedItemId == None:
            self.__mCanvas.tag_bind(
                self.__draggedItemId, "<Motion>", self.__onMove
            )
            self.__mCanvas.tag_bind(
                self.__draggedItemId, "<ButtonRelease-1>", self.__onRelease, "+"
            )

    def onPressName(self, event):
        self.onPressDot(event)
        self.__mCanvas.tag_bind(
            self.__draggedItemId + 1, "<Motion>", self.__onMove
        )
        self.__mCanvas.tag_bind(
            self.__draggedItemId + 1, "<ButtonRelease-1>", self.__onRelease, "+"
        )
    ...
    def __onRelease(self, event):
        ...
        self.__mCanvas.tag_bind(
            self.__draggedItemId, "<ButtonPress-1>", self.onPressDot
        )
        self.__mCanvas.tag_bind(
            self.__draggedItemId + 1, "<ButtonPress-1>", self.onPressName
        )
        self.__draggedItemId = None
        ...
        for key in coreDict:
            ...
            if (
                midX >= coordinates[0]
                and midX <= coordinates[2]
                and midY >= coordinates[1]
                and midY <= coordinates[3]
            ):
                peName = key
            else:
                pass
        ...
        if (
            not peName == None
            and not peName in self.__mApiInstance.getUsedCores()
        ):
            ...
            if mappingToChange != None:
                ...
                raise RuntimeWarning(
                    "Something went wrong during dropping the item!"
                )
        else:
