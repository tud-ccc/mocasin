...
# development imports
from mocasin.gui.dataTemplates import mappingInformation, PlatformInformation
from mocasin.gui.managers import drawManager
...
class drawAPI:
    ...
    def __init__(self, canvas, border, scaling, width, height, **kwargs):
        ...
        self.__mDrawManager = drawManager(
            self, canvas, border, scaling, width, height, **kwargs
        )
    ...
    def addMapping(self, mapping, color="default"):
        ...
        :param Mapping mapping: The mocasin mapping object that should be applied.
        ...
        if self.__mPlatform == None:
            raise RuntimeWarning(
                "Can not add a mapping if no platform is specified"
            )
        else:
            ...
            if mappingColor != None:
                ...
                raise RuntimeWarning(
                    "Could not add mapping  because no color was available!"
                )
        return key
    ...
    def setColorVectors(
        self,
        platformVector=["default"],
        mappingVector=["default"],
        peVector=["default"],
    ):
        ...
        """
        self.__mDrawManager.setColorVectors(
            platformVector, mappingVector, peVector
        )
