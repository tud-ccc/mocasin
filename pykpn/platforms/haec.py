...
class DesignerPlatformHAEC(Platform):
    def __init__(self, processor_0, name='haec-like',symmetries_json=None,embedding_json=None):
        ...
        for i in range(4):
            ...
            processors = [f"processor_{j:04d}" for j in range(i*16,(i+1)*16)]
            topology = meshTopology(processors)
            designer.createNetworkForCluster(f"cluster_{i}", 'optic', topology, sd, 6000000.0, 100, 150, 100, 60)


        for i in range(4-1):
            designer.addCommunicationResource(f"wireless_{i}",
        ...
        designer.addCommunicationResource(f"RAM", [f"cluster_{i}" for i in range(4)],
                                          2000,
                                          3000,
                                          float('inf'),
                                          float('inf'),
