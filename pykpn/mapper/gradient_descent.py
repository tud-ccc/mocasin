...
class GradientDescentMapper(object):
    ...
    def __init__(self, kpn, platform, trace, representation, gd_iterations=100,
                 ...
                 jobs=2,momentum_decay=0.5,parallel_points=5):
    ...
    def generate_mapping(self):
        ...
        for _ in range(self.parallel_points):
            ...
            if hasattr(self.representation,'canonical_operations') and not self.representation.canonical_operations:
                m = self.representation.toRepresentationNoncanonical(mapping_obj)
            else:
        ...
        for _ in iterations_range:
            ...
            for i in active_points:
               grads[i] = self.momentum_decay * old_grads[i]\
            ...
            for idx,i in enumerate(active_points):
                ...
                mappings[i] = mappings[i] + gammas[idx] * (-grads[i]) * self.stepsize
                log.debug(f"moving mapping {i} to: {mappings[i]}")
                mappings[i] = self.representation.approximate(np.array(mappings[i]))
            ...
            if cur_exec_times[idx] < self.best_exec_time:
                log.info(f"better than old best time ({self.best_exec_time}). Replacing")
                self.best_exec_time = cur_exec_times[idx]
                self.best_mapping = mappings[idx]
            ...
            for i in active_points:
                ...
                # stuck in a loop.
                if np.allclose(mappings[i],last_mappings[i]) or \
                        np.allclose(mappings[i],before_last_mappings[i]):
