from pykpn.mapper.fair import StaticCFS,gen_trace_summary
...
class MockTraceGenerator(object):
    ...
    def next_segment(self,proc_name,core_type):
        ...
        if self.num_lookups[(proc_name,core_type)] >= self.max_length:
            ...
            return TraceSegment(process_cycles=self.lookup((proc_name,core_type)))
...
def test_gen_trace_summary(kpn,platform):
    ...
    trace = MockTraceGenerator(proc_names,core_types,const_func,max_length=num_iter)
    ...
    for core in core_types:
        for proc in kpn.processes():
            single_comb = lambda x : (x[0] == core)*(x[1] == proc.name)*const_value
            trace = MockTraceGenerator(proc_names, core_types, single_comb, max_length=num_iter)
            trace_summary = gen_trace_summary(kpn, platform, trace)
