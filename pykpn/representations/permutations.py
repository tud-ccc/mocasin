        ...
        for l in clist_old:
            ...
            for _ in range(0,times):
                for t in sorted(trivials):
                    if start_bound == t:
                        start_bound+=1
                new_l += list(map(lambda cycles : list(map(lambda x : x + start_bound , cycles)),l))
                #print("start bound: " + str(start_bound))
                start_bound += g.n
            generators_clist.append(new_l)

        generators = list(map( lambda cycles : Permutation(cycles,times*g.n+len(trivials),action=action), generators_clist) )
