...
class LRSolver:
    ...
    Dual function is
    g(lambda) = min_x Lagr(x,lambda) = sum_{i=1}^n min_{x_i} { e(x_i) + sum_{k=1}^m lambda_k * r_k(x_i) } - sum_{k=1}^m lambda_k*R_k
    ...
    The problem is solved by decomposition into a master problem and several subproblems by applying a subgradient method.

    Stefan Wildermann, Michael Glaß, and Jürgen Teich. 2014. Multi-objective distributed run-time resource management for many-cores.
    In Proceedings of the conference on Design, Automation & Test in Europe (DATE '14).
    """
    def __init__(self, platform, constraints = LRConstraint.RESOURCE, rounds = 1000, params = None):
    ...
    def job_config_cost(job, config, l):
        ...
        assert isinstance(config, CanonicalMapping), "Config must be a type of CanonicalMapping, but it is {}".format(type(config))
        ...
        if l_r is not None:
            res += (l_r * LRSolver.__job_config_resource(job, config)).reduce(lambda x, y: x + y)
        if l_rdp is not None:
            res += (l_rdp * LRSolver.__job_config_rdp(job, config)).reduce(lambda x, y: x + y)
        return res
    ...
    def __job_min_cost(job, l):
        ...
        configs = [(name, can_mapping, LRSolver.job_config_cost(job, can_mapping, l))
                for name, can_mapping in job.app.mappings.items() if name != "__idle__"]
        min_name, min_config, min_cost = min(configs,
                                  key=lambda x: x[2])
    ...
    def solve(self, jobs):
        ...
        if self.__relax_rdp:
            window = max([job.deadline for job in jobs if job.deadline != math.inf], default = math.inf)
        ...
        for t in range(1, self.__max_rounds + 1):
            ...
            # Initiate results for found job configs [ (job, config_name, config) ]
            # TODO: Remove explicit config_name
            min_configs = []

            for job in jobs:
                # Application subproblem
                job_config_name, job_config, min_cost = LRSolver.__job_min_cost(job, l)
                min_configs.append(tuple((job, job_config_name, job_config)))
                if self.__verbose:
                    log.debug("Job rid = {}, config = {}[{}], time = {}, deadline = {}, f = {}"
                              .format(job.rid, job_config_name, job_config.core_types, job_config.time(start_cratio = job.cratio), job.deadline, min_cost))
                # Calculate subgradient of delay coefficients
                if self.__relax_d:
                    if job.deadline == math.inf:
                        new_l_d[job.rid] = 0.0
                    else:
                        delta = LRSolver.__job_config_delay(job, job_config) - job.deadline
                        new_l_d[job.rid] = max(0.0, l[0][job.rid] + delta * self.__step_size_delay(t))
            ...
            if not changed:
                if self.__verbose:
                    log.debug("New lambda value is the same as previous. Stopping iterating.")
                break
