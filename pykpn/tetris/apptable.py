...
class CanonicalMapping:
    ...
    Canonical mapping is a mapping which represents a set of equivalent
    mappings, which can be derived by applying transformations and keeps
    ...
    Args:
        ...
        kpn_graph (pykpn.slx.kpn.SlxKpnGraph): the kpn graph of the
            application
        platform (Platform): the platform
    """
    def __init__(self, mapping: dict, time: float, energy: float,
                 kpn_graph: SlxKpnGraph, platform: Platform):
        self.__time = time
        ...
        if kpn_graph is None:
            ...
            self.__mapping = CanonicalMapping.__create_kpn_mapping(
                kpn_graph, platform, mapping)
    ...
    @staticmethod
    def __create_kpn_mapping(kpn_graph: SlxKpnGraph, platform: Platform,
                             mapping: dict):
        """Create KPN mapping from a dict, the simple vector representation.

        Priority and policy chosen at random, and scheduler chosen randomly from
        the possible ones. Channels are chosen at random.
        ...
        for p in kpn_graph.processes():
            assert p.name in mapping, (
                "Process {} is not in mapping = {}".format(p.name, mapping))
            pe_name = mapping[p.name]
            pe = platform.find_processor(pe_name)
            schedulers = [
                sched for sched in all_schedulers if pe in sched.processors
            ]
    ...
    def __calc_core_types(self, platform):
        if self.__mapping is not None:
            ps = {
                self.__mapping.affinity(p)
                for p in self.__mapping.kpn.processes()
            }
        else:
            ps = {}
...
class Application:
    ...
    def __init__(self, name: str, path: str, platform: Platform,
                 add_idle=False):
    ...
    def best_case_energy(self, start_cratio=0.0, end_cratio=1.0):
        return min([
            v.energy(start_cratio=start_cratio, end_cratio=end_cratio)
            for k, v in self.mappings.items() if k != "__idle__"
        ])

    def best_case_time(self, start_cratio=0.0, end_cratio=1.0):
        return min([
            v.time(start_cratio=start_cratio, end_cratio=end_cratio)
            for k, v in self.mappings.items() if k != "__idle__"
        ])
