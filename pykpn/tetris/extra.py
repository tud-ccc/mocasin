...
class NamedDimensionalNumber:
    """This class defines multi-dimensional number (similar to vector),
    all dimensions have unique names.

    The operations between instances of the class NamedDimensionalNumber are
    valid iff. they have the same dimensions.
    ...
    def __check_object_same_type(self, other):
        ...
        if set(self.__dict.keys()) != set(other.__dict.keys()):
            print("Error: trying to use a binary operator on "
                  "NamedDimensionalNumbers with different dimensions")
    ...
    def max_per_dim(cls, v1, v2):
        """Returns the new number with maximum value between v1 and v2
        at each dimenstion.
        """
