...
class generator():
            ...
            for i in range(0, length):
                if self._mappingIdentifier == []:
                    ...
                    if operation == 0:
                        ...
                        tmp = self._SharedCoreOperation(usedRunningTogether, usedRunningTogether)
                        result += tmp[0]
    ...
    def _SharedCoreOperation(self, usedIdentifier, usedRunningTogether):
        ...
        for i in range(0, amount):
            rnd = randrange(0, len(self._taskIdentfier))
            taskIdentifier = self._taskIdentfier[rnd]
            while taskIdentifier in usedRunningTogether or taskIdentifier in usedIdentifier or taskIdentifier in currentlyUsed:
                j += 1
                rnd = randrange(0, len(self._taskIdentfier))
                taskIdentifier = self._taskIdentfier[rnd]
                if j >= 30:
