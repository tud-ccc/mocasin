...
for i, sigma in enumerate( sigmas):
    ...
    print("i: "+ str(i) +" sigma: "+ str(sigma) +" n_tot: " + str(n_tot) + ", p: " + str(p))
    
    points = map( lambda x : (x - int(n_tot * p + 0.5) + mu)  % n, np.random.binomial(n_tot,p,trials) )
    xs = []
    for j in range(0,n):
    ...
    axes[i].set_xlim(0,n)
    #print(points)
    print("calculated mean: " + str(np.mean(points)) + ", deviation: " + str(np.std(points,ddof=1)))
